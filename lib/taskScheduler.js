...
var TaskScheduler = function(config) {
  ...
  var allSpecs = ConfigParser.resolveFilePatterns(
      ConfigParser.getSpecs(config), false, config.configDir).filter(function(specInfo) {
      return excludes.indexOf(specInfo.spec) < 0;
    });
  ...
  config.multiCapabilities.forEach(function(capabilities) {
    ...
    if (capabilities.exclude) {
      ...
      capabilitiesSpecs = ConfigParser.resolveFilePatterns(
          capabilitiesSpecs).filter(function(specInfo) {
            var filter = false;
            for(var i = 0; i < capabilitiesSpecExcludes.length; i++) {
              if (capabilitiesSpecExcludes[i].spec == specInfo.spec)
                filter = true;
            }
            return !filter;
        });
