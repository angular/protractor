 ...
 * @example
 * // Returns the DIV for the cat, but not dog.
 * var cat = element(by.cssContainingText('.pet', 'CAT', true));
 * @param {string} cssSelector A css selector
 * @param {string} searchText Text to search for in the element's content
 * @param {boolean=} opt_ignoreCase If true, the content text will be matched ignoring case.
 */
ProtractorBy.prototype.cssContainingText = function(cssSelector, searchText, opt_ignoreCase) {
  return {
    findElementsOverride: function(driver, using) {
      return driver.findElements(
        webdriver.By.js((opt_ignoreCase === true)
				? clientSideScripts.findByCssContainingTextIgnoreCase 
