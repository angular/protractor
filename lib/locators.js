...
/**
 * Find elements inside an ng-repeat. Does a partial match, so any elements bound to
 * variables containing the input string will be returned.
 *
 * @view
 ...
 *
 * // You can also use a substring for a partial match
 * var divs = element.all(by.repeater('cat'));
 *
 ...
 * 
 * // You can also use a substring for a partial match
 * var divs = element.all(by.repeater('book'));
 */
    ...
    findElementsOverride: function(driver, using, rootSelector) {
      return driver.findElements(
        webdriver.By.js(clientSideScripts.findAllRepeaterRows,
            repeatDescriptor, false, using, rootSelector));
    },
    toString: function toString() {
        ...
        findElementsOverride: function(driver, using, rootSelector) {
          return driver.findElements(
            webdriver.By.js(clientSideScripts.findRepeaterRows,
                repeatDescriptor, false, index, using, rootSelector));
        },
        toString: function toString() {
            ...
            findElementsOverride: function(driver, using, rootSelector) {
              return driver.findElements(
                  webdriver.By.js(clientSideScripts.findRepeaterElement,
                      repeatDescriptor, false, index, binding, using, rootSelector));
            },
            toString: function toString() {
              return 'by.repeater("' + repeatDescriptor + '").row("' + index +
                '").column("' + binding + '")';
        ...
        findElementsOverride: function(driver, using, rootSelector) {
          return driver.findElements(
              webdriver.By.js(clientSideScripts.findRepeaterColumn,
                  repeatDescriptor, false, binding, using, rootSelector));
        },
        toString: function toString() {
...
/**
 * Find elements inside an ng-repeat by exact repeater name.
 *
 * @view
 * <div ng-repeat="cat in pets">
 *   <span>{{cat.name}}</span>
 ...
 * @example
 * // Returns the DIV for the second cat.
 * var secondCat = element(by.exactRepeater('cat in pets').row(1));
 *
 * // Returns the SPAN for the first cat's name.
 * var firstCatName = element(by.exactRepeater('cat in pets').
 *     row(0).column('{{cat.name}}'));
 *
 * // Returns a promise that resolves to an array of WebElements from a column
 * var ages = element.all(
 *     by.exactRepeater('cat in pets').column('{{cat.age}}'));
 *
 * // Returns a promise that resolves to an array of WebElements containing
 * // all top level elements repeated by the repeater. For 2 pets rows resolves
 * // to an array of 2 elements.
 * var rows = element.all(by.exactRepeater('cat in pets'));
 *
 * // Returns a promise that resolves to an array of WebElements containing all
 * // the elements with a binding to the book's name.
 * var divs = element.all(by.exactRepeater('book in library').column('book.name'));
 *
 * // Returns a promise that resolves to an array of WebElements containing
 * // the DIVs for the second book.
 * var bookInfo = element.all(by.exactRepeater('book in library').row(1));
 *
 * // Returns the H4 for the first book's name.
 * var firstBookName = element(by.exactRepeater('book in library').
 *     row(0).column('{{book.name}}'));
 *
 * // Returns a promise that resolves to an array of WebElements containing
 * // all top level elements repeated by the repeater. For 2 books divs
 * // resolves to an array of 4 elements.
 * var divs = element.all(by.exactRepeater('book in library'));
 */
ProtractorBy.prototype.exactRepeater = function(repeatDescriptor) {
  return {
    findElementsOverride: function(driver, using, rootSelector) {
      return driver.findElements(
        webdriver.By.js(clientSideScripts.findAllRepeaterRows,
            repeatDescriptor, true, using, rootSelector));
    },
    toString: function toString() {
      return 'by.repeater("' + repeatDescriptor + '")';
    },
    row: function(index) {
      return {
        findElementsOverride: function(driver, using, rootSelector) {
          return driver.findElements(
            webdriver.By.js(clientSideScripts.findRepeaterRows,
                repeatDescriptor, true, index, using, rootSelector));
        },
        toString: function toString() {
          return 'by.repeater(' + repeatDescriptor + '").row("' + index + '")"';
        },
        column: function(binding) {
          return {
            findElementsOverride: function(driver, using, rootSelector) {
              return driver.findElements(
                  webdriver.By.js(clientSideScripts.findRepeaterElement,
                      repeatDescriptor, true, index, binding, using, rootSelector));
            },
            toString: function toString() {
              return 'by.repeater("' + repeatDescriptor + '").row("' + index +
                '").column("' + binding + '")';
    ...
    column: function(binding) {
      return {
        findElementsOverride: function(driver, using, rootSelector) {
          return driver.findElements(
              webdriver.By.js(clientSideScripts.findRepeaterColumn,
                  repeatDescriptor, true, binding, using, rootSelector));
        },
        toString: function toString() {
          return 'by.repeater("' + repeatDescriptor + '").column("' +
            binding + '")';
