...
var Protractor = function(webdriverInstance, opt_baseUrl, opt_rootElement) {
  ...
  /**
   * When synchronizationType is 'protractor', and synchronization is true,
   * Protractor will 'waitForAngular' before performing actions.
   * When synchronization is false, Protractor will not wait
   * until $timeouts and $http calls have been processed, which can cause
   * tests to become flaky. This can be used when necessary, such as
   * when a page continuously polls an API using $timeout, or when
   * interacting with non-angular pages.
   *
   * Use 'setSynchronization' and 'getSynchronization' in tests to set/get the
   * synchronization variables within the context of the controlFlow.
   *
   * @example:
   * browser.setSynchronization(false);
   * browser.get(non-angular page);
   * element(by.css('input[value="login"]')).click();
   * browser.setSynchronization(true);
   * browser.get (angular page);
   *
   * Note: synchronizationType is a string so you can define custom types in
   * your page objects, and handle their actions as needed
   * @example:
   * pageObject.click = function() {
   *   browser.getSynchronization().then(function(sync) {
   *    if (sync === 'custom_webdriver') {
   *       self.waitReady();
   *     }
   *     self.click();
   *   });
  ...
  this.synchronizationType = 'protractor'

  this.setSynchronization = function(sync) {
    var self = this;
    this.driver.controlFlow().execute(function() {
      if (typeof sync === 'boolean') {
        ...
        self.synchronizationType = sync;
      }
    });
  };

  this.getSynchronization = function() {
    var self = this;
    return this.driver.controlFlow().execute(function() {
      return self.synchronization ? self.synchronizationType : self.synchronization;
    });
  };
...
Protractor.prototype.waitForAngular = function(opt_description) {
  ...
  var self = this;
  return this.getSynchronization().then(function(sync) {
    if (sync !== 'protractor') {
...
Protractor.prototype.get = function(destination, opt_timeout) {
  ...
  return this.getSynchronization().then(functions(sync) {
    if (sync !== protractor) {
