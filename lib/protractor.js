...
/**
 * Perform the navigation specified in "navFn" and loads mock modules before
 * Angular. Assumes that the page being loaded uses Angular.
 * If you need to access a page which does not have Angular on load, use
 * the wrapped webdriver directly.
 *
 * @param {function} navFn The function to wrap around and synchronize.
 * @param {number=} opt_timeout Number of seconds to wait for Angular to start.
 */
Protractor.prototype.synchronizedNavigate = function(navFn, opt_timeout) {
  var self = this;
  ...
  return self.driver.executeScript(
    ...
    navFn();
    // At this point, Angular will pause for us if not already loaded, 
    ...
    var assertAngularOnPage = function(arr) {
      ...
      return self.driver.executeScript('return window.name;').then(
          function(windowName) {
        if (windowName === oldWindowname.replace(DEFER_LABEL, '')){
          // Angular reloaded
          ...
          // Angular did not reload
          return self.driver.executeScript(
            'window.name = window.name.replace("' + DEFER_LABEL + '", "");'
            );
        } 
      });
    };
    ...
    return self.driver.executeAsyncScript(
...
/**
 * See Protractor.prototype.synchronizedNavigate.
 *
 * Navigate to a destination. This always performs a full refresh (i.e Angular 
 * is reloaded even if you navigating here from the same app).
 * Allow Angular to bootstrap and mock modules to load.
 *
...
Protractor.prototype.get = function(destination, opt_timeout) {
  var self = this;
  destination = url.resolve(self.baseUrl, destination);

  if (self.ignoreSynchronization) {
    return self.driver.get(destination);
  }

  self.driver.get('about:blank');
  
  return self.synchronizedNavigate(function() {
    self.driver.executeScript('window.location.replace("' + destination + '");');
  }, opt_timeout);  
};
 ...
 * See Protractor.prototype.synchronizedNavigate.
 *
 * Navigate forward as you would normally in a browser. This means that if you
 * are going to the same app, Angular is not reloaded, but if you are going to
 * a different or new app, Angular is loaded.
 * Allow Angular to bootstrap and mock modules to load.
 *
 * @param {number=} opt_timeout Number of seconds to wait for Angular to start.
 */
Protractor.prototype.forward = function(opt_timeout) {
  ...
  if (self.ignoreSynchronization) {
    return self.driver.navigate().forward();
  }
 ...
 * See Protractor.prototype.synchronizedNavigate.
 *
 * Navigate back as you would normally in a browser. This means that if you
 * are going to the same app, Angular is not reloaded, but if you are going to
 * a different or new app, Angular is loaded.
 * Allow Angular to bootstrap and mock modules to load.
 *
...
Protractor.prototype.back = function(opt_timeout) {
  ...
  if (self.ignoreSynchronization) {
    return self.driver.navigate().back();
  }
 ...
 * See Protractor.prototype.synchronizedNavigate.
 *
 * Refresh the browser as you would normally in a browser. This means Angular
 * is always reloaded. 
 * Allow Angular to bootstrap and mock modules to load.
 *
 * @param {number=} opt_timeout Number of seconds to wait for Angular to start.
