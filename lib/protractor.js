...
var buildElementHelper = function(ptor) {
   ...
   */
  var ElementArrayFinder = function(locator, opt_parentElementFinder, opt_filterFn) {
    if (!locator) {
      throw new Error('Locator cannot be empty');
    }
    this.locator_ = locator;
    this.parentElementFinder_ = opt_parentElementFinder || null; 
    this.opt_filterFn_ = opt_filterFn; 
  };
  ...
  ElementArrayFinder.prototype.getWebElements = function() {
    ...
    // First get all web elements that satisfy the chain of locators
    var unfilteredWebElems;
    if (this.parentElementFinder_) {
      ...
      if (this.locator_.findElementsOverride) {
        unfilteredWebElems = 
          this.locator_.findElementsOverride(ptor.driver, parentWebElement);
      } else {
        unfilteredWebElems = parentWebElement.findElements(this.locator_);
      }
    } else {
      unfilteredWebElems = ptor.waitForAngular().then(function() {
    ...
    // Then filter as specified by this.opt_filterFn_
    if (this.opt_filterFn_) {
      return unfilteredWebElems.then(function(arr) {
        var list = [];
        arr.forEach(function(webElem, index) {
          var wrappedWebElem = new ElementFinder(webElem);
          self.opt_filterFn_(wrappedWebElem, index).then(function(satisfies) {
            if (satisfies) {
              list.push(webElem);
            }
          });
  ...
  ElementArrayFinder.prototype.get = function(index) {
    return new ElementFinder(this.locator_, this.parentElementFinder_, null, index, this.opt_filterFn_);
  };
  ...
  ElementArrayFinder.prototype.asElementFinders_ = function() {
    return this.getWebElements().then(function(arr) {
      ...
      arr.forEach(function(webElem) {
        list.push(new ElementFinder(webElem));
      });
      return list; 
    });
  };
  ...
  ElementArrayFinder.prototype.filter = function(filterFn) {
    var self = this;
    var compositeFilterFn;
    if (this.opt_filterFn_) {
      compositeFilterFn = function(elem, index) {
        return webdriver.promise.fullyResolved(
            [filterFn(elem), self.opt_filterFn_(elem, index)]).
      ...
      compositeFilterFn = filterFn;
    }

    return new ElementArrayFinder(
        this.locator_, this.parentElementFinder_, compositeFilterFn);
  };
   ...
   */
  var ElementFinder = function(locatorOrElement, opt_parentElementFinder, 
      opt_actionResult, opt_index, opt_filterFn) {

    if (!locatorOrElement) {
      throw new Error ('Locator cannot be empty');
    }

    this.locatorOrElement_ = locatorOrElement;
    this.parentElementFinder_ = opt_parentElementFinder || null;
      ...
      if(!self[fnName]) {
        self[fnName] = function() {
          ...
          return new ElementFinder(
              locatorOrElement, opt_parentElementFinder, 
              actionResult, opt_index, opt_filterFn);
        };
      }
    });
  };
  ...
  ElementFinder.prototype.isPresent = function() {
    var isPresent;
    if (this.locatorOrElement_ instanceof webdriver.WebElement) {
      isPresent = this.locatorOrElement_.isPresent();
    } else {
      isPresent = new ElementArrayFinder(
          this.locatorOrElement_, this.parentElementFinder_, this.opt_filterFn_).
              count().then(function(count) {
                return !!count;
              });
    }
    return new ElementFinder(this.locatorOrElement_, 
        this.parentElementFinder_, isPresent, this.opt_index_, this.opt_filterFn_);
  };
  ...
  ElementFinder.prototype.getWebElement = function() {
    if (this.locatorOrElement_ instanceof webdriver.WebElement) {
      return this.locatorOrElement_; //locatorOrElement_ is webelement
    }

    var self = this; 
    var locator_ = this.locatorOrElement_;
    var webElementsPromise = new ElementArrayFinder(
        locator_, this.parentElementFinder_, this.opt_filterFn_).getWebElements();

    var id = webElementsPromise.then(function(arr) {
      var locatorMessage = locator_.toString();
      if (!arr.length) {
  ...
  ElementFinder.prototype.evaluate = function(expression) {
    ...
    return new ElementFinder(this.locatorOrElement_, this.parentElementFinder_, 
      evaluatedResult, this.opt_index_, this.opt_filterFn_);
  };
  ...
  ElementFinder.prototype.allowAnimations = function(value) {
    ...
    return new ElementFinder(this.locatorOrElement_, this.parentElementFinder_, 
      allowAnimationsResult, this.opt_index_, this.opt_filterFn_);
  };
...
Protractor.prototype.isElementPresent = function(locatorOrElement) {
  return this.element(locatorOrElement).isPresent();
};
