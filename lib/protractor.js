...
/**
 * Set how long Protractor waits for the page to finish loading Angular
 *
 * @param {number} waitForAngularTimeout The timeout in milliseconds.
 */
Protractor.prototype.setWaitForAngularTimeout = function(waitForAngularTimeout) {
  this.getPageTimeout = waitForAngularTimeout;
};
...
Protractor.prototype.waitForAngular = function() {
  ...
  this.driver.executeAsyncScript(clientSideScripts.testForAngular, 
        Math.floor(this.getPageTimeout / 1000), false).
      then(function(angularTestResult) {
        var hasAngular = angularTestResult[0];
        if (!hasAngular) {
          var message = angularTestResult[1];
          throw new Error('Angular could not be found on the page : ' + message);
        }
      }, function(err) {
 ...
 * @param {!string} url_ The url for which the mock modules are injected into.
 * @param {...*} varArgs Any additional arguments will be provided to
 *     the script and may be referenced using the `arguments` object.
 */
Protractor.prototype.addMockModule = function(name, script, url_) {
  var moduleArgs = Array.prototype.slice.call(arguments, 3);
 ...
 * @param {!string} url_ The url for which the mock modules are injected into.
 */
Protractor.prototype.clearMockModules = function(url_) {
  this.mockModules_ = [];
  this.doBootstrap_(url_);
};
 ...
 * @param {!string} url_ The url for which the mock modules are injected into.
 */
Protractor.prototype.removeMockModule = function(name, url_) {
...
/**
 * Check to see if a variable contains a function.
 * @param {Function} functionToCheck A variable potentially representing a function.
...
/**
 * Navigate to a page and bootstrap mock modules into it. 
 * @param {!string} url_ The url for which the mock modules are injected into.
 */
Protractor.prototype.doBootstrap_ = function(url_) {
  var timeout = this.getPageTimeout;
  var self = this;

  url_ = url.resolve(this.baseUrl, url_);
  ...
  for (var i = 0; i < this.mockModules_.length; ++i) {
    ...
    moduleNames.push(mockModule.name);
    var jsonfiedArgs = JSON.stringify(mockModule.args);
    var script = 'var args = JSON.parse(\'' + jsonfiedArgs + '\');';
    if (isFunction_(mockModule.script)) {
      script += '(' + mockModule.script + ').apply(null, args);';
    } else {
      script += mockModule.script;
    }
    modules.push({
 ...
 return self.driver.executeScript(
    clientSideScripts.injectModulesAndResumeBootstrap, modules).
        then(function(injectModulesResult) {
          if (!injectModulesResult[0]) {
            var message = injectModulesResult[1];
            throw new Error('Cannot inject modules onto the page ' +
                url_ + ' : ' + message);
          }
        });
};
 ...
 *
 */
Protractor.prototype.refresh = function() {
  this.driver.navigate().refresh();
  return this.waitForAngular();
};

/**
 * See webdriver.WebDriver.back
 *
 * Navigate back from the current page and loads mock modules before
 * Angular. Assumes that the page being loaded uses Angular.
 * If you need to access a page which does not have Angular on load, use
 * the wrapped webdriver directly.
 *
 */
Protractor.prototype.back = function() {
  this.driver.navigate().back();
  return this.waitForAngular();
};

/**
 * See webdriver.WebDriver.forward
 *
 * Navigate forward from the current page and loads mock modules before
 * Angular. Assumes that the page being loaded uses Angular.
 * If you need to access a page which does not have Angular on load, use
 * the wrapped webdriver directly.
 *
 */
Protractor.prototype.forward = function() {
