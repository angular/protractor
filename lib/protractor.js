 ...
 *     scope.
 */
var Protractor = function(webdriverInstances, opt_baseUrl, opt_rootElement) {
  
  /**
   * An array of drivers.
   * @see {this.driver}
   *
   * @type {Array<webdriver.WebDriver>}
   */
  this.driverList = webdriverInstances.constructor === Array ? 
      webdriverInstances : [webdriverInstances];
  ...
  for (var method in this.driver) {
    if(!this[method] && typeof this.driver[method] == 'function') {
      if (methodsToSync.indexOf(method) !== -1) {
        mixin(this, this.driver, method, this.waitForAngular.bind(this));
      } else {
        mixin(this, this.driver, method);
      }
    }
  }
  var self = this;
...
Protractor.prototype.switchToDriver = function(index) {
  var self = this;
  return webdriver.promise.controlFlow().execute(function() {
    if (index >= self.driverList.length) {
      throw 'Cannot switch to driver ' + index +'. Only ' + self.driverList.length + 
          ' drivers instantiated as defined in the config.';
    };
    self.driver = self.driverList[index];
  });
};
