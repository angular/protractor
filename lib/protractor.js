...
/**
 * Add multiple modules to load before Angular whenever Protractor.get is called.
 * Modules will be registered after existing modules already on the page,
 * so any module registered here will override preexisting modules with the same
 * name.
 *
 * @param {!Array} modules an array containing module objects in the format:
 * {name: 'name', scripts: functionReference}
 */
Protractor.prototype.addMockModules = function(modules) {
  for (var i = 0, l = modules.length; i < l; i ++) {
    var module = modules[i];
    this.moduleNames_.push(module.name);
    this.moduleScripts_.push(module.script);
  }
};
