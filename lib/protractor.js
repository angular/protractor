...
Protractor.prototype.pause = function(opt_debugPort) {
  ...
  // Helper used only by './wddebugger.js' to insert code into the
  // control flow. 
  // In order to achieve this, we maintain a promise at the current stage of
  // the flow execution at all times, so that we can insert frames into it.
  ...
  this.dbgCodeExecutor_ = {
    ...
    execute: function(code) {
      ...
      self.execPromise_ = self.execPromise_.then(function() {
    ...
    getResult: function() {
      ...
      // JSON doesn't know about undefined. 
      return this.execPromiseResult_ === undefined ? 
        undefined : JSON.stringify(this.execPromiseResult_);
    }
  };
