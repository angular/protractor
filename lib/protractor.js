...
var buildElementHelper = function(ptor, opt_usingChain) {
  ...
  element.all = function(locator) {
    ...
    /**
     * Calls the input function on each WebElement found by the locator,
     * returns true if input function return true once at least, otherwise
     * returns false
     *
     * @alias element.all(locator).some(someFunction)
     * @view
     * <ul class="items">
     *   <li>First</li>
     *   <li>Second</li>
     ...
     *
     * @example
     * var hasTarget = element.all(by.css('.items li')).some(function(element) {
     ...
     *
     * @param {function(webdriver.WebElement)} fn Input function
     * @return {!webdriver.promise.Promise} A promise that resolves to a boolean
     *     that is true if at least one time input function returned true.
     */
    elementArrayFinder.some = function(someFn) {
      return using().findElements(locator).then(function(arr) {
        ...
        for(index = 0; index < arr.length && deferred.isPending(); index++) {
          webElem = arr[index];
          someResult = someFn(webElem, index);
          webdriver.promise.fullyResolved(someResult).then((function (isLastItem){
