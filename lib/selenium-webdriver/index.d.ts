...
// tslint:disable:variable-name 3rd party
// tslint:disable:interface-name 3rd party
...
export namespace error {
   ...
   */
  // tslint:disable-next-line:no-any Runtime type testing.
  function encodeError(err: any): {error: string, message: string};
}


/**
 * Typings for lib/logging.js
 */
export namespace logging {
  ...
  class Entry {
     ...
     */
    constructor(
        level: Level|string|number, message: string, opt_timestamp?: number,
   ...
   * dot-separated naming scheme. For instance, 'foo' is considered the parent
   * of the 'foo.bar' and an ancestor of 'foo.bar.baz'.
   *
  ...
  class Logger {
    ...
    handlers_: Set<(entry: Entry) => void>;

    /** @return {string} the name of this logger. */
    getName(): string;

    /**
     * @param {Level} level the new level for this logger, or `null` if the
     *     logger should inherit its level from its parent logger.
     */
     ...
     * @return {boolean} whether messages recorded at the given level are
     *     loggable by this instance.
     */
    isLoggable(level: Level): boolean;

    /**
     * Adds a handler to this logger. The handler will be invoked for each
     * message logged with this instance, or any of its descendants.
     *
     ...
     */
    removeHandler(handler: (entry: Entry) => void): void;
...
export class Capabilities {
  ...
  // tslint:disable-next-line:no-any TS does not support typing JSON
  toJSON(): any;
...
export class Window {
  ...
  /**
   * Google3 modification for v4 API backport.
