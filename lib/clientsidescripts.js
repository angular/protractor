...
functions.findBindings = function(binding, exactMatch, using) {
  ...
  for (var i = 0; i < bindings.length; ++i) {
    ...
    if (dataBinding) {
      var bindingNames = [];
      if (angular.isArray(dataBinding)) {
        bindingNames = dataBinding;
      } else {
        var singleBinding = dataBinding.exp || dataBinding[0].exp || dataBinding;
        bindingNames.push(singleBinding);
      }
      for (var j = 0; j < bindingNames.length; ++j) {
        if (exactMatch) {
          var matcher = new RegExp('({|\\s|$|\\|)' + binding + '(}|\\s|^|\\|)');
          if (matcher.test(bindingNames[j])) {
            matches.push(bindings[i]);
          }
        } else {
          if (bindingNames[j].indexOf(binding) != -1) {
            matches.push(bindings[i]);
          }
        }
      }
