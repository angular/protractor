...
/**
 * Find elements by css selector and textual content ignoring case.
 *
 * @param {string} cssSelector The css selector to match.
 * @param {string} searchText The exact text to match ignoring case.
 * @param {Element} using The scope of the search. Defaults to 'document'.
 *
 * @return {Array.<Element>} An array of matching elements.
 */
functions.findByCssContainingTextIgnoreCase = function(cssSelector, searchText, using) {
  var using = using || document;
  var elements = using.querySelectorAll(cssSelector);
  var matches = [];
