 ...
 */
 functions.findRepeaterRows = function(repeater, exactMatch, index, using) {
  using = using || document;
  ...
  for (var p = 0; p < prefixes.length; ++p) {
    ...
    for (var i = 0; i < repeatElems.length; ++i) {
      if (exactMatch) {
        if (repeatElems[i].getAttribute(attr).split(' track', 1)[0].
          split(' |', 1)[0] == (repeater)) {
 ...
 */
 functions.findAllRepeaterRows = function(repeater, exactMatch, using) {
  using = using || document;
 ...
 */
functions.findRepeaterElement = function(repeater, exactMatch, index, binding, using, rootSelector)
{
  var matches = [];
  var root = document.querySelector(rootSelector || 'body');
  using = using || document;
 ...
 */
functions.findRepeaterColumn = function(repeater, exactMatch, binding, using, rootSelector) {
  var matches = [];
  var root = document.querySelector(rootSelector || 'body');
  using = using || document;
