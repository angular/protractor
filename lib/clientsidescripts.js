...
/**
 * FFind elements by css selector and textual content.
 *
 * @param {string} ngEvent An event that's angular ddirective
 * @param {string} value A value to match the event
 *
 * @return {Array.<Element>} The matching elements.
 */
functions.findByNgevent = function(anEvent, value, using) {
    using = using || document;

  var prefixes = ['ng-', 'ng_', 'data-ng-', 'x-ng-', 'ng\\:'];
  for (var p = 0; p < prefixes.length; ++p) {
    var selector = '[' + prefixes[p] + anEvent + '.value(' + value + ')"]';
    var elements = using.querySelectorAll(selector);
    if (elements.length) {
      return elements[0];
    }
