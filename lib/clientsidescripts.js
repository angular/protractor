 ...
 * @param {boolean} requireResumeBootstrap Whether the bootstrap function 
 *     resumeBootstrap need to be present in angular.
 * @param {function} asyncCallback callback
 */
functions.testForAngular = function(attempts, requireResumeBootstrap, asyncCallback) {
  ...
  var check = function(n) {
    try {
      if (window.angular && (!requireResumeBootstrap || window.angular.resumeBootstrap)) {
        callback([true, null]);
      } else {
        if (n >= 1) {
...
/**
 * Inject mock modules into angular and call resumeBootstrap.
 *
 * @param {Array.<Object>} mockModules Mock modules to inject (contains name
 *    and script).
 */
functions.injectModulesAndResumeBootstrap = function(mockModules) {
  if(typeof(Storage) !== 'undefined') {
    ...
    for (var i = 0 ; i < mockModules.length; ++i) {
      ...
      try {
        /*jshint evil:true */
        eval(mockModules[i].script);
      } catch(err) {
        return [false, 'Error while running module script ' + 
                mockModules[i].name + ': ' + err.message];
      }
    }
    sessionStorage.angularInjectedModules = JSON.stringify(mockModules);
    angular.resumeBootstrap(moduleNames);
    return [true, null]; 
  } else {
    return [false, 'browser does not support web storage'];
  }
};
