 ...
 * @param {boolean} exactMatch The searchText should match exactly or not.
 * @param {Element} using The scope of the search.
 *
 * @return {Array.<Element>} An array of matching elements.
 */
functions.findByCssContainingText = function(cssSelector, searchText, exactMatch, using) {
  using = using || document;
  ...
  for (var i = 0; i < elements.length; ++i) {
    ...
    var elementMatches = !exactMatch && searchText instanceof RegExp ?
