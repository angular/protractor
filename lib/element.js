 ...
 * step can be negative in which case we go backwards.
 *
 ...
 * Note that slice(null,null,-1) will NOT produce a reversed
 * array - use reverse() on the result instead.
 *
 * @view
 * <ul class="items">
 *   <li>Zeroth</li>
 ...
 *
 * expect(element.all(locator).slice(1,4).getText()).toEqual(
 *  ['First','Second','Third']);
 * expect(element.all(locator).slice(2,-1).getText()).toEqual(
 *  ['Second','Third','Fourth','Fifth','Sixth']);
 * expect(element.all(locator).slice(6).getText()).toEqual(
 * ['Sixth','Seventh']);
 * expect(element.all(locator).slice(-1,-5,-2).getText()).toEqual(
 * ['Seventh','Fifth']);
 *
 * @param {number=} sliceStart start of the slice. Defaults to 0.
 * @param {number=} sliceStop end of the slice(not included). Defaults to
 * one past the last element.
 * @param {number=} sliceStep The jump. Defaults to 1.
...
ElementArrayFinder.prototype.slice = function (sliceStart, sliceStop, sliceStep)
{
  ...
  var getWebElements = function () {
    return self.getWebElements().then(function (parentWebElements) {
      var start = (sliceStart === undefined || sliceStart === null) ?
          0 : sliceStart;
      var stop = (sliceStop === undefined || sliceStop === null) ?
          parentWebElements.length : sliceStop;
      var step = (sliceStep === undefined || sliceStep === null
      || sliceStep === 0) ? 1 : sliceStep;
      ...
      if (start > parentWebElements.length) {
        throw new Error('Start ' + start + ' > result length '
            + parentWebElements.length);
      }
      if (stop > parentWebElements.length) {
        throw new Error('Stop ' + stop + ' > result length '
            + parentWebElements.length);
      }
      ...
      if (step < 0) {
        ...
        if (start < stop) {
          throw new Error('Start ' + start + ' < Stop ' + stop +
              ' for negative step value : ' + step);
        }
      } else {
        // If step is positive and start is greater than stop, error out.
        if (start > stop) {
          throw new Error('Start ' + start + ' > Stop ' + stop +
              ' for positive step value: ' + step);
        }
      }
...
ElementArrayFinder.prototype.asElementFinders_ = function() {
  ...
  return this.getWebElements().then(function(arr) {
    return arr.map(function(webElem) {
      return ElementFinder.fromWebElement_(self.ptor_, webElem, self.locator_);
    });
  });
};
