  ...
  var getWebElements = function() {
    return self.getWebElements().then(function(parentWebElements) {
      ...
      if (i < 0) {
        ...
        index = i = parentWebElements.length + i;
      }
      if (i < 0 || i >= parentWebElements.length) {
        var error = new Error('Index out of bound. Trying to access element at ' +
            'index: ' + index + ', but there are only ' +
            parentWebElements.length + ' elements that match locator ' +
            self.locator_.toString());
        error.code = webdriver.error.ErrorCode.NO_SUCH_ELEMENT;
        throw error;
      }
