...
var init = function(configFile, additionalConfig) {
   ...
   * @param {boolean} realtimeReporting Whether to report results of test in 
   *     real time. 
   * @return {RunnerFork}
   */
  var RunnerFork = function(task) {
    ...
    this.reporter = reporter.addSessionReporter(task, this.process.pid, false);
  };
   ...
   * call back function to be called when a test completes.
   *
...
var reporter = {
  ...
  addSessionReporter: function(task, pid, realtimeReporting) {
    var taskReporter = new TaskReporter_(task, pid, realtimeReporting);
 ...
 * @param {pid} PID of process running the task.
 * @param {boolean} realtimeReporting Whether to report results of test in 
 *     real time.   
 * @return {TaskReporter_}
 */
var TaskReporter_ = function(task, pid, realtimeReporting) {
  this.task = task;
  this.pid = pid;
  this.failedCount = 0;
  this.buffer = '';
...
/**
 * Report the following data. If realtimeReporting is true, the data will be
 * printed to screen immediately. Otherwise, the data will be saved to a buffer
 * until it is flushed by the function testsDone. 
 *
 * @private
 * @param {string} data 
 */
TaskReporter_.prototype._log = function(data) {
