...
var init = function(configFile, additionalConfig) {
  ...
  helper.runFilenameOrFn_(config.configDir, config.beforeLaunch).then(function() {
    ...
    var createNextRunnerFork = function() {
      ...
      if (task) {
        var RunnerFork = require('./RunnerFork');
        var runnerFork = new RunnerFork(configFile, additionalConfig, task, forkProcess);

        runnerFork.run().then(function(result) {
    ...
    deferred.promise.then(function() {
      ...
      if (config.testResultOutput) {
        taskResults_.saveResults(config.testResultOutput);
      }
...
//###### REPORTER #######//
/**
 * Keeps track of a list of test results. Provides method to add a new 
 * result and to aggregate the results into a summary. 
 */
var taskResults_ = {
  results_: [],
