...
var init = function(configFile, additionalConfig) {
  ...
  process.on('exit', function(code) {
    ...
    } else if (scheduler && scheduler.numTasksOutstanding() > 0) {
  ...
  helper.runFilenameOrFn_(config.configDir, config.beforeLaunch).then(function() {
    var resolveCapabilities = [];
    if (config.capabilities) {
      var capPromise = q.when(config.capabilities).
          then(function(capabilities) {
            config.capabilities = capabilities;
          });
      resolveCapabilities.push(capPromise);
    }
    if (config.multiCapabilities) {
      var multiCapPromise = q.all(config.multiCapabilities).
          then(function(multiCapabilities) {
            config.multiCapabilities = multiCapabilities;
          });
      resolveCapabilities.push(multiCapPromise);  
    }
    return q.all(resolveCapabilities);
  }).then(function() {
