...
var Runner = function(config) {
  ...
  config.numDrivers = config.numDrivers > 0 ? config.numDrivers : 1;
 ...
 * @private
 */
Runner.prototype.setupGlobals_ = function(drivers) {
  ...
  // All the drivers are of the same capabilities, just multiplied.
  drivers[0].getCapabilities().then(function(caps) {
...
Runner.prototype.run = function() {
  ...
  var gracefulShutdown = function() {
    var deferredArray = [];
    if (drivers) {
      deferredArray = drivers.map(function(driver) {
  ...
  // 3) Setup globals and plugins
    ...
    self.setupGlobals_.bind(self)(drivers);
  ...
  // 4) Execute test cases
    ...
    return require(frameworkPath).run(self, specs).then(function(testResults) {
      return helper.joinTestLogs(setupResults, testResults);
    });
  // 5) Teardown plugins
  }).then(function(testResults) {
    var deferred = q.defer();
    plugins.teardown().then(function(teardownResults) {
      deferred.resolve(helper.joinTestLogs(testResults, teardownResults));
    }, function(error) {
