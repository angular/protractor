...
var setUpSelenium = function() {
  ...
  if (config.chromeOnly) {
    ...
    if (!config.seleniumServerJar) {
      ...
      var defaultStandalone = path.resolve(
        __dirname,
      ...
      throw new Error(
        'there is no selenium server jar at the specified '+
...
var runTests = function() {
  if (config.jasmineNodeOpts.specFolders) {
    throw new Error(
      'Using config.jasmineNodeOpts.specFolders is deprecated ' +
  ...
  if (config.chromeOnly) {
    driver = chrome.createDriver(
      new webdriver.Capabilities(config.capabilities),
      new chrome.ServiceBuilder(config.chromeDriver).build()
    );
  } else {
