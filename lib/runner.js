    ...
    for (var i = 0; i < source.length; i++) {
        ...
        if (filenameOrFn) {
          if (typeof filenameOrFn === 'function') {
            ...
            var args = filenameOrFn.split(' '),
              file = args.shift();
            if(args.length > 0) {
              require(path.resolve(this.config_.configDir, file))(args);
            } else {
              var startup = require(path.resolve(this.config_.configDir, file));
              if(startup instanceof Function) {
                startup();
              }
            }
          } else {
