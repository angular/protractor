...
var SessionScheduler = function(config) {
  ...
  config.multiCapabilities.forEach(function(capability) {
    ...
    capability.count = capability.count ? capability.count : 1;
    if (config.splitTestsBetweenCapabilities && capability.count > 1) {
      console.log('Running with splitTestsBetweenCapabilities - ' +
        'config.capabilities.count will be ignored');
      capability.count = 1;
    }
...
SessionScheduler.prototype.nextSession = function() {
  for (var i = 0; i < this.sessionQueues.length; ++i) {
    ...
    if (q.curInst < q.maxInst && q.specsIndex < q.specsList.length) {
      var sessionId = rotatedIndex + 1;
      if (q.specsList.length > 1) {
        sessionId += String.fromCharCode(97 + q.specsIndex); //ascii 97 is 'a'
      }
      ++q.curInst;
