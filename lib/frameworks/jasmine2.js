...
exports.run = function(runner, specs) {
  ...
  // Filter specs/suite to run based on jasmineNodeOpts.grep and jasmineNodeOpts.invert.
  jasmine.getEnv().specFilter = function(spec, suite) {
    var suiteMatched = isSuiteMatched(suite);

    //If it matched at suite level and invert is true, all the specs will not be included.
    if(suiteMatched && jasmineNodeOpts.invertGrep){
      spec.pend();
      return true;
    }

    //If it matched at suite level and invert is false, all the specs will be included.
    if(suiteMatched && !jasmineNodeOpts.invertGrep){
      return true;
    }

    var specWithSuiteFullName = spec.getFullName();
    if(suite){
      specWithSuiteFullName += suite.getFullName();
    }

    var grepMatch = !jasmineNodeOpts ||
        !jasmineNodeOpts.grep ||
        specWithSuiteFullName.match(new RegExp(jasmineNodeOpts.grep, 'i')) != null;
  ...
  function isSuiteMatched(suite) {
    if (!suite) {
      return false;
    }
    var grepMatch = !jasmineNodeOpts || !jasmineNodeOpts.grep ||
        suite.getFullName().match(new RegExp(jasmineNodeOpts.grep)) != null;
