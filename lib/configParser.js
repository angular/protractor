...
/**
 * Expands the combinations of capabilities & specs to a list of configs with
 * only one spec & capability each.
 *
 * @return {Array} An array of config objects
 */
ConfigParser.prototype.flatten = function() {
  var original = this.config_;

  // Use capabilities if multiCapabilities is empty.
  if (!original.multiCapabilities.length) {
    original.multiCapabilities = [original.capabilities];
  }

  var list = _.map(original.multiCapabilities, function (cap) {
    var newConfig = _.cloneDeep(original);
    newConfig.capabilities = cap;
    delete newConfig.multiCapabilities;
    return newConfig;
  });
