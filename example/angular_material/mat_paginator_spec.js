describe('angular-material paginator component page', () => {
  ...
  it('Should navigate to next page', async() => {
    ...
    expect(await $('.mat-paginator-range-label').getAttribute('innerText')).toEqual('11 - 20 of 100');
  });

  it('Should navigate to previous page', async() => {
    await $('button[aria-label=\'Previous page\']').click();

    expect(await $('.mat-paginator-range-label').getAttribute('innerText')).toEqual('1 - 10 of 100');
  });
  ...
  it('Should change list length to 5 items per page', async() => {
    ...
    expect(await $('.mat-paginator-range-label').getAttribute('innerText')).toEqual('1 - 5 of 100');
  });
});